...
def run_main_routine_loop(video_path):
    ...
    for result in results:
        ...
        if fame_counter == 1:
            # ---------------------------------------------------------lab3---------------------------------------------------
            # get the width and height from the shape of result.orig_img
            height, width, channels = [0, 0, 0]
            height, width, channels = result.orig_img.shape

            # save the first frame from result.orig_img as 'background.png'  using cv2.imwwrite
            cv2.imwrite('background.png', result.orig_img)
...
def anomaly_detection(tracked_objects, routine_map, result, height=None, width=None, out=None):
    ...
    # create  the detection frame and send a copy of the routine map to prevent contamination
    _, detection_frame = build_detections_and_routine_map(tracked_objects, copy.deepcopy(routine_map), width, height)
...
def run_main_anomaly_loop(video_name_routine=None):
    ...
    if os.path.exists('routine_map.pkl')==False:
        result = next(results)
        routine_map = np.ones((result[0].orig_img.shape[0], result[0].orig_img.shape[1]), dtype=np.float32)
        routine_map[0,0] = 10
    else:
    ...
    for result in results:
        ...
        save_tracked_objects_to_csv(tracked_objects, csv_filename=csv_filename)
...
def track_objects(result, bboxes, labels, confidences, tracked_objects, bbox_path):
    ...
    # extract the current_ids from the result,boxes.id only if its not None!!!
...
def init_system(video_path):
    ...
    csv_filename = f'gt_tracked_objects.csv'

    # create a list of csv_header with the following  'bbox', 'track_id', 'object_name', 'time_date', 'confidence',
    # 'bbox_image_path', 'score', 'tag'
    csv_header = ['bbox', 'track_id', 'object_name', 'time_date', 'bbox_image_path', 'confidence', 'score','tag']

    # Create a csv if not exists using pandas dataframe to_csv function on the csv_header list
...
# (4) go to the git repo and find the problem!!!

# (5)change the code such that you will get 100 in your grade!
#--------------------------------- Lab 2 -----------------------------------------
...
if __name__ == "__main__":
    ...
    download_missing_video_files(video_name_routine=video_name_routine,
                                 video_link=params_links.routine_frame_link())
    ...
    download_missing_video_files(video_name_routine=video_name_test,
                                 video_link=params_links.test1_link())
    run_main_anomaly_loop(video_name_test)
    #---------------------------------  Lab 3 ------------------------------------------

    #---------------------------------  Project  -----------------------------------------

    # video_name_routine = os.path.join(current_direcotry, 'grass_routine.mp4')
    # download_missing_video_files(video_name_routine=video_name_routine,
    #                              video_link=params_links.grass_routine_link())
    # run_main_routine_loop(video_name_routine)
    # video_name_test = os.path.join(current_direcotry, 'grass_anomaly.mp4')
    # download_missing_video_files(video_name_routine=video_name_test,
    #                              video_link=params_links.grass_anomaly_link())
    # run_main_anomaly_loop(video_name_test)
    # video_name_routine = os.path.join(current_direcotry, 'concrete_routine.mp4')
    # download_missing_video_files(video_name_routine=video_name_routine,
    #                              video_link=params_links.concrete_routine_link())
    # run_main_routine_loop(video_name_routine)
    # video_name_test = os.path.join(current_direcotry, 'concrete_anomaly.mp4')
    # download_missing_video_files(video_name_routine=video_name_test,
    #                              video_link=params_links.concrete_anomaly_link())
    # run_main_anomaly_loop(video_name_test)
    # video_name_routine = os.path.join(current_direcotry, 'hall_routine.mp4')
    # download_missing_video_files(video_name_routine=video_name_routine,
    #                              video_link=params_links.hall_routine_link())
    # run_main_routine_loop(video_name_routine)
    # video_name_test = os.path.join(current_direcotry, 'hall_anomaly.mp4')
    # download_missing_video_files(video_name_routine=video_name_test,
    #                              video_link=params_links.hall_anomaly_link())
    # run_main_anomaly_loop(video_name_test)
    #---------------------------------  Project -----------------------------------------
